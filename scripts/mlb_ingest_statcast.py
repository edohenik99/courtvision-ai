"""Explicit local-file CLI for the Phase 3C Statcast ingestion prototype."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Parse a local Statcast CSV for historical MLB research."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = ingest_local_statcast_csv(
        args.input_csv,
    ...
    summary = {
        ...
        "manifest_output_path": (
            str(result.manifest_output_path) if result.manifest_output_path else None
        ),
