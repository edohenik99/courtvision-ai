...
def test_fragility_attribution_report_generation(tmp_path: Path) -> None:
    ...
    shadow_meta = payload.get("fragility_survivability_shadow_eval", {})
    shadow_path = Path(shadow_meta.get("path", ""))
    assert shadow_path.exists()
    shadow = json.loads(shadow_path.read_text(encoding="utf-8"))
    assert shadow.get("readiness_verdict") == "not_ready_insufficient_sample"
    assert shadow.get("conflict_analysis", {}).get("high_confidence_high_fragility", 0) >= 0
