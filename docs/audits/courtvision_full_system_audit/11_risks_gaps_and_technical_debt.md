...
| --- | --- | --- | --- |
| F-001 | Foundation implemented, not operational | Active v2-only `OfficialPick`; isolated `LegacyOfficialPickV1`; append-only operator review; writer-locked batch authorization; one-review/one-pick enforcement; recomputed promotion identity; exact provenance; lexically writer-owned review commits; immutable authorization mappings with no writable instance storage; exact schema dispatch; flattened structural-safe report DTOs; `OfficialPickSettlement`; and settlement-correction contracts. | Operator identity is asserted rather than authenticated; no operational approval interface or automatic promotion exists; existing settlement/grading consumers still need migration; no historical IDs or settlements were guessed or backfilled. |
| F-002 | Partially implemented | Market observations and model candidates have explicit non-pick record kinds; operator review freezes candidates and separates approved/rejected/deferred/expired states; promotion requires committed approval; official-pick and settlement reports retain strict joins; MLB/NBA runtimes do not auto-promote or auto-settle. | Existing MLB observation grader remains separately labeled observation analysis; no automated official-pick performance workflow or paper trial is active. |
...
| F-020 | Medium | Testing | Operator-promotion focused tests cover real-writer batch authorization, one-review/one-pick replay/conflict behavior, exact promotion identity and provenance, generic review authoring, absence of an instance-level prepared commit primitive, callback recursion/failure rollback, lock presence at every commit hook, process races, active-v1 rejection, legacy isolation, schema dispatch, `object.__setattr__` immutability attacks, and structural report leakage; broader repository validation remains required for each release candidate. | A focused pass does not establish operational or live readiness. | Keep focused, sharded root, stable, type, compile, and schema checks release-gated. |
