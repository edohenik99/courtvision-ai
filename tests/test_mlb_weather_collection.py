...
from courtvision.sports.mlb.data_collection.weather_collector import WEATHER_FILENAME
...
def _stadium_map(path: Path, *, park_id: str = "NYC21") -> Path:
    path.write_text(
        "park_id,stadium_name,latitude,longitude,timezone,elevation_m\n"
        f"{park_id},Yankee Stadium,40.8296,-73.9262,America/New_York,17\n",
        encoding="utf-8",
    )
    return path
...
def test_fetch_weather_writes_hourly_raw_csv_and_manifest_evidence(
    ...
    assert result.manifest.collector_version == "1.3.0"

    weather = next(
        source for source in result.manifest.sources if source.source_name == "weather_meteostat"
    )
    ...
    assert by_name["weather_meteostat"]["provider"] == "https://meteostat.net/"
    assert by_name["weather_meteostat"]["source_notes"]
