...
$runner = (Join-Path $automationRoot "run_mlb_hr_dynamic_closing.ps1")
...
$logFile = Join-Path `
    $logDir `
    "closing_scheduler_$dayStamp.log"
...
try {
    ...
    $desiredRunUtc =
        $earliestStartUtc.AddMinutes(-45)
    ...
    $taskName =
        "CourtVision MLB HR Dynamic Closing $dayStamp $controlId"
    ...
    $null = Assert-RecoveryNewTask -TaskName $taskName -TaskPath $taskPath -Execute $taskExecutable -Arguments $actionArguments -WorkingDirectory $repo
    if ($DryRun) {
        Write-Log (
    ...
    Register-ScheduledTask `
        ...
        -Trigger $trigger `
        -Settings $settings `
        -Description "Explicit-date fresh MLB HR research closing snapshot." `
        |
        Out-Null
